...
class TestSimpleML(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        print("=== hw0 - simple machine learning test start... ===")
    
    @classmethod
    def tearDownClass(cls) -> None:
        print("=== hw0 - simple mahcine learning test end. ===")
    
    def test_add(self):
        self.assertEqual(add(5, 6), 11)
        self.assertEqual(add(3.2,1.0), 4.2)
        self.assertIsInstance(add(4., 4), float)
        
    def test_parse_mnist(self):
        X, y = parse_mnist("data/train-images-idx3-ubyte.gz",
                           "data/train-labels-idx1-ubyte.gz")
        self.assertEqual(X.dtype, np.float32)
        self.assertEqual(y.dtype, np.uint8)
        self.assertEqual(X.shape, (60000, 784))
    ...
    def test_softmax_regression_epoch(self):
    ...
    def test_softmax_regression_epoch_cpp(self):
        # test numeical gradient
        np.random.seed(0)
        X = np.random.randn(50,5).astype(np.float32)
        y = np.random.randint(3, size=(50,)).astype(np.uint8)
