...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # raise NotImplementedError()   
    for cur in reverse_topo_order: 
        # get all out node from current 
        out_list = node_to_output_grads_list.get(cur, None)
        assert out_list != None
        
        # sum the gradients of all contributing nodes
        cur.grad = sum_node_list(out_list)
        
        # if cur is input node, then skip 
        if not cur.inputs:
            continue
        
        # calculate the gradient contribution of current node to all in-degree nodes
        in_grad = list(cur.op.gradient_as_tuple(cur.grad, cur))
        
        for _in_node, _grad in zip(cur.inputs, in_grad):
            if not node_to_output_grads_list.get(_in_node, None):
                node_to_output_grads_list[_in_node] = [_grad]
            else:
                node_to_output_grads_list[_in_node].append(_grad)
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    topo_order = []
    topo_sort_dfs(node_list[0], list(), topo_order)
